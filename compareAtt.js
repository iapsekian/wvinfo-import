...
let dataPreparation = () => {
	...
	let wait4DataPreparationEnd = () => {
		count--
		if(!count){
			fs.writeFileSync('./log/wikiDataAtt.json', JSON.stringify(wikiData))
			fs.writeFileSync('./log/wikiDataTitleAtt.json', JSON.stringify(wikiDataTitle))
			fs.writeFileSync('./log/bookurDataAtt.json', JSON.stringify(bookurData))
			compareAttData()
		}
	}
	...
	MongoClient.connect(mdbUrl4BookUr, (err, db) => {
		...
		cltContents.find({typeId: '57ea19736d0e81454c7b23d2', online: online}).project({_id: 1, text: 1}).toArray()
		.then( d => {
			bookurData = d
			db.close()
			wait4DataPreparationEnd()
...
let compareAttData = async () => {
	console.log('Compare Attractions starts.....')
	let tmpLevel = {}
    ...
    fs.writeFileSync('./log/compareAttLevel.json', JSON.stringify(level));
    fs.writeFileSync('./mapping/compareAttResult.json', JSON.stringify(result));
    fs.writeFileSync('./mapping/compareAttNoResult.json', JSON.stringify(noResult));
}
